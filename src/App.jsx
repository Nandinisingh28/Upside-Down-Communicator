...
function App() {
  ...
  // Initialize Konami code detector ONCE
  useEffect(() => {
  ...
  // Main Effect: Stage Monitoring
  useEffect(() => {
    // Detector is already initialized in the mount effect
    ...
    // Sync sequence: Ensure it exists in Stage 3
    if (mode === 'possessed' && recoveryStage === 3 && recoverySequence.length === 0 && konamiDetectorRef.current) {
      konamiDetectorRef.current.regenerateSequence();
      setRecoverySequence(konamiDetectorRef.current.getSequenceDisplay());
    } else if (mode !== 'possessed' && recoverySequence.length > 0) {
      setRecoverySequence([]);
    }

    const handleKeyDown = (e) => {
      // Only allow input in Stage 3
      console.log(`[DEBUG] KeyDown: ${e.code}, Mode: ${mode}, Stage: ${recoveryStage}`);

      if (mode === 'possessed' && recoveryStage < 3) return;

      if (konamiDetectorRef.current) {
        const result = konamiDetectorRef.current.handleKey(e.code);
        console.log(`[DEBUG] Handler Result: ${result}, Current Progress: ${konamiDetectorRef.current.getProgress()}`);
